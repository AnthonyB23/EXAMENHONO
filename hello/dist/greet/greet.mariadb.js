...
export class Greet {
    ...
    // STATS
    static async stats() {
        const total = await connection.query('SELECT COUNT(*) AS total FROM regards');
        const byLanguage = await connection.query('SELECT language, COUNT(*) AS count FROM regards GROUP BY language');
        return {
            total: total[0].total,
            byLanguage
        };
